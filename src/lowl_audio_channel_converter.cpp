...
Lowl::AudioFrame Lowl::AudioChannelConverter::to_stereo(AudioFrame p_audio_frame) const {
    p_audio_frame[0] = p_audio_frame[0];
    p_audio_frame[1] = p_audio_frame[0];
    return p_audio_frame;
}

Lowl::AudioFrame Lowl::AudioChannelConverter::to_mono(AudioFrame p_audio_frame) const {
...
Lowl::AudioChannelConverter::convert(std::vector<AudioFrame> p_audio_frames, const ConvertFn p_convert_fn) const {
...
std::vector<Lowl::AudioFrame>
Lowl::AudioChannelConverter::convert(Lowl::AudioChannel p_from, Lowl::AudioChannel p_to, std::vector<AudioFrame> audio_data,
                                     Error error) const {
    if (p_from == p_to) {
...
Lowl::AudioChannelConverter::convert(AudioChannel p_to, std::shared_ptr<AudioData> p_audio_data, Error error) const {
