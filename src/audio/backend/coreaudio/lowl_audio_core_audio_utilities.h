...
namespace Lowl::Audio {
    ...
    public:
        ...
        get_stream_ids(AudioObjectID p_device_id, AudioObjectPropertyScope p_scope, Lowl::Error &error);
        ...
        static SampleRate get_output_sample_rate(AudioUnit _Nonnull p_audio_unit, Lowl::Error &error);

        static SampleRate get_device_default_sample_rate(AudioObjectID p_device_id, Lowl::Error &error);

        static SampleCount get_latency_low(UInt32 desired_size,
                                           AudioObjectID p_device_id,
                                           AudioStreamID p_stream_id,
                                           AudioObjectPropertyScope p_scope,
                                           Lowl::Error &error);

        static SampleCount get_latency_high(AudioObjectID p_device_id,
                                            AudioStreamID p_stream_id,
                                            AudioObjectPropertyScope p_scope,
                                            Lowl::Error &error);

        static SampleCount
        get_device_latency(AudioObjectID p_device_id, AudioObjectPropertyScope p_scope, Lowl::Error &error);

        static SampleCount
        get_safety_offset(AudioObjectID p_device_id, AudioObjectPropertyScope p_scope, Lowl::Error &error);

        static SampleCount
        get_stream_latency(AudioStreamID p_stream_id, AudioObjectPropertyScope p_scope, Lowl::Error &error);

        static SampleCount
        get_buffer_frame_size(AudioObjectID p_device_id, AudioObjectPropertyScope p_scope, Lowl::Error &error);

        static AudioValueRange
        get_buffer_frame_size_range(AudioObjectID p_device_id, AudioObjectPropertyScope p_scope, Lowl::Error &error);

        static void set_buffer_frame_size(AudioObjectID p_device_id,
                                          AudioObjectPropertyScope p_scope,
                                          UInt32 p_frames_per_buffer,
                                          Lowl::Error &error);

        static void set_maximum_frames_per_slice(AudioUnit _Nonnull p_audio_unit,
                                                 AudioUnitScope p_scope,
                                                 AudioUnitElement p_element,
                                                 UInt32 p_maximum_frames_per_slice,
                                                 Lowl::Error &error);

        static SampleCount get_maximum_frames_per_slice(AudioUnit _Nonnull p_audio_unit,
                                                        AudioUnitScope p_scope,
                                                        AudioUnitElement p_element,
                                                        Lowl::Error &error);

        static void set_render_quality(AudioUnit _Nonnull p_audio_unit,
                                       AudioUnitScope p_scope,
                                       AudioUnitElement p_element,
                                       UInt32 p_render_quality,
                                       Lowl::Error &error);

        static void add_property_listener(AudioObjectID p_device_id,
        ...
        static Lowl::SampleCount
        set_frames_per_buffer(AudioObjectID p_device_id, SampleCount p_frames_per_buffer, Lowl::Error &error);

        static AudioStreamBasicDescription
        get_audio_stream_description(AudioUnit _Nonnull p_audio_unit, AudioUnitScope p_scope, Lowl::Error &error);
    };
} // namespace Lowl::Audio
